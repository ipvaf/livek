...
export function LiveAuctionCard({ auction }: { auction: Auction }) {
  const isIG = auction.platform === "instagram"
    ...
    <Link href={`/auctions/${auction.id}`} className="block auction-card">
      <div className="bg-white border border-border rounded-2xl overflow-hidden">
        ...
        <div className="relative h-48 bg-surface">
          ...
          <div className={`absolute bottom-2 right-2 h-6 w-6 rounded-full flex items-center justify-center text-white ${isIG ? "bg-instagram" : "bg-tiktok"}`}>
